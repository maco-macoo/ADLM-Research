"""Lightweight Lightning diffusion module for language modeling
with multiple parameterizations, samplers, and evaluations.
Developed upon https://github.com/kuleshov-group/remdm/blob/main/diffusion.py.
"""
...
class Diffusion(L.LightningModule):
  def __init__(
    ...
    self.denoiser = models.denoiser.DIT(
            self.config, vocab_size=self.vocab_size
            )
  ...
  def load_state_dict(self, state_dict, strict: bool = True):
    """Load weights, remapping legacy `replica` keys if necessary."""
    if any(key.startswith('replica.') for key in state_dict.keys()):
      remapped_state_dict = {}
      for key, value in state_dict.items():
        if key.startswith('replica.'):
          new_key = key.replace('replica.', 'denoiser.', 1)
  ...
  def forward(self, x, sigma):
    """Run the forward diffusion step to obtain denoised logits from noisy inputs.
    Args:
      x (torch.Tensor): Input tensor containing partially masked tokens.
      sigma (Union[torch.Tensor, float]): Noise level(s) associated with the current diffusion step.
    Returns:
      Union[
        torch.Tensor,
        Tuple[torch.Tensor, torch.Tensor],
      ]: The predicted logits in the default parameterization, or:
        - (logits, base_logits) when using ADLM with 'subs' parameterization,
        - logits transformed according to the 'sedd' parameterization,
        - logits transformed according to the 'd3pm' parameterization,
        depending on the configured parameterization mode.
    
    """
    sigma = self._process_sigma(sigma)
    with torch.amp.autocast('cuda', dtype=torch.float32):
      # Apply anchor transformer to get base logits
      base_logits = self.backbone(x, sigma)
      # Apply denoiser transformer to get denoised logits
      logits = self.denoiser(base_logits, sigma)

    if self.parameterization == 'subs':
      logits = self._subs_parameterization(logits=logits,
              xt=x)
  ...
  def _ddpm_caching_update(self, x, t, dt, p_x0=None, conf=None):
    ...
    if p_x0 is None:
      logits, base_logits = self.forward(x, sigma_t)
      p_x0 = logits.exp()
      if self.config.sampling.nucleus_p < 1:
        sorted_probs, sorted_indices = torch.sort(p_x0, descending=True, dim=-1)
        cumulative_probs = torch.cumsum(sorted_probs, dim=-1)
    ...
    if self.config.sampling.sampler == 'mdlm':
      ...
      q_xs = torch.where(copy_flag.unsqueeze(-1), q_xs, q_xs_2)
      xs = _sample_categorical(q_xs)
    elif self.config.sampling.sampler == 'remdm-rescale':
      alpha_t = (1 - move_chance_t)[0].item()
      alpha_s = (1 - move_chance_s)[0].item()
      if alpha_t > 0:
  ...
  def _ddpm_update(self, x, t, dt):
    ...
    log_p_x0, logt_p_base_x0 = self.forward(x, unet_conditioning)
  ...
  def _sample(self, num_steps=None, eps=1e-5):
    ...
    if self.config.sampling.noise_removal:
      ...
      if self.sampler == 'analytic':
        ...
        x = self.forward(x, unet_conditioning)[0].argmax(dim=-1)
    return x
  ...
  def get_score(self, x, sigma):
    model_output, anchor_output = self.forward(x, sigma)
    if self.parameterization == 'subs':
  ...
  def _reconstruction_loss(self, x0):
    ...
    model_output_t0, anchor_output_t0 = self.forward(x0, unet_conditioning)
    return - torch.gather(input=model_output_t0,
                          dim=-1,
                          index=x0[:, :, None]).squeeze(-1)

  def _forward_pass_diffusion(self, x0, prefix):
    """Run one forward diffusion step and compute the training loss or score term.
    Args:
        x0 (torch.Tensor): Discrete token tensor of shape (batch, seq_len) used as the
            ground-truth sequence for supervision.
        prefix (str): Identifier for the current phase (e.g., ``"train"``) used to
            control anchor loss logging.
    Returns:
        torch.Tensor: Scalar loss per element in the batch, combining diffusion,
        reconstruction, and anchor components depending on the current
        parameterization and training configuration.
    """
    ...
    model_output, anchor_output = self.forward(xt, unet_conditioning)
    ...
    # compute CE loss for anchor tokens
    if self.config.enable_anchor_loss and prefix == 'train':
      # Normalize the logits such that logits.exp() is
      # a probability distribution over vocab_size.
      anchor_output_normalized = (
        anchor_output - torch.logsumexp(anchor_output, dim=-1,
                                        keepdim=True))

      # considers all tokens as important
      log_p_phi = torch.gather(
        input=anchor_output_normalized,
        dim=-1,
        index=x0[:, :, None]).squeeze(-1)
      
      # create mask to only compute loss on anchor tokens
      anchor_masks = [
        self.create_sequence_mask(x0[i], self.config.threshold)
        for i in range(x0.shape[0])
      ]
      anchor_mask = torch.stack(anchor_masks).to(x0.device)

      # compute scaling factor for anchor loss
      progress = self.global_step / self.config.trainer.max_steps
      base_scaling_factor = min(
        self.config.base_scaling_factor1,
        progress * self.config.base_scaling_factor2)

      # compute total loss
      log_p_theta = log_p_theta + base_scaling_factor * log_p_phi * anchor_mask
      
      if (self.global_step+1) % self.config.trainer.log_every_n_steps == 0:
          self.log(name='trainer/base_scaler',
                  value=base_scaling_factor,
                  on_step=True,
                  on_epoch=False,
                  sync_dist=True)
  ...
  def create_sequence_mask(self, sequence, threshold):
    """
    Create a binary mask for the sequence where tokens that appear more than
    'threshold' times are marked as 0, and others as 1.
    input:
        sequence: torch.Tensor of shape (seq_length,)
        threshold: int, frequency threshold
    output:
        mask: torch.Tensor of shape (seq_length,), binary mask
    """
    token_counts = collections.Counter(sequence.tolist())
    return torch.tensor(
      [0 if token_counts[token.item()] > threshold else 1 for token in sequence],
      dtype=torch.long)
  ...
  def _loss(self, x0, attention_mask, prefix):
    ...
    if self.parameterization == 'ar':
      ...
      loss = self._forward_pass_diffusion(input_tokens, prefix)
    ...
    for _ in range(num_strides + 1):
      ...
      for i in range(num_steps + 1):
        ...
        if (not torch.allclose(x_next, x)
          ...
          sampling_steps += 1
        x = x_next
      x = self.forward(x, 0 * ones)[0].argmax(dim=-1)
